...
function Game() {
  ...
  const MAX_ROWS = 5;
  const [arrayCharsInRow, setArrayCharsInRow] = useState<string[]>([]);
  ...
  const getSecretWordFromServer = async () => {
      ...
      catch( e )
      {
          ...
          alert("Error: Failed to get secret word from server. " + (e as {message: string}).message)
      }
  ...
  function onClickedChar(clickedChar: string) {
  ...
  return (
    ...
    >
      ...
      <div className="main-div">
        ...
        <Keyboard 
            onClickedChar={onClickedChar} 
            arrayCharsInRow={arrayCharsInRow}
            secretWord={secretWord}
